...
[
  {
    "id": "default",
    "slugs": ["sample-spot", "sample-music-video"]
  }
]
```

Schema requires `min(1).max(5)` — flexible so dev data doesn't have to fake duplicates. HeroCycler should handle a 1-item list gracefully (it just won't cycle).
